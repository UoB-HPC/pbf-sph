...
public:
  ...
  sph::Result<T, N, V> advance(const sph::SphParams<T, N, V> &config, //
    ...
    std::vector<PartiallyAdvected<T, N, V>> mirror(xs.size());
    ...
    foreach_1d(xs.size(), [&](size_t i) {
      ...
      mirror[i].particle = p;
      mirror[i].particle.velocity = combinedForce * config.dt + mirror[i].particle.velocity;

      auto pStar = (mirror[i].particle.velocity * config.dt) + (mirror[i].particle.position / config.scale);
      mirror[i].pStar = pStar;
      mirror[i].zIndex = zCurveGridIndexAtCoordAt(N(mirror[i].pStar.x() - minExtent.x()), //
                                                  N(mirror[i].pStar.y() - minExtent.y()), //
                                                  N(mirror[i].pStar.z() - minExtent.z()), //
                                                  h);
    });
    advect();
    ...
    std::sort(mirror.begin(), mirror.end(), [](auto &l, auto &r) { return l.zIndex < r.zIndex; });
    sortz();

    auto gridtable = watch.start("CPU gridtable");
    auto hostGridTable = makeGridTable(extent.x(), extent.y(), extent.z(), mirror.size(),
                                       [&mirror](auto i) { return mirror[i].zIndex; });
    const auto gridTableN = hostGridTable.size();
    gridtable();
      ...
      for (V<3, N> offset : NEIGHBOUR_OFFSETS) {
        ...
        if (zIdx < gridTableN && zIdx + 1 < gridTableN) {
          for (size_t a = hostGridTable[zIdx]; a < hostGridTable[zIdx + 1]; a++) {
            if (mirror[a].particle.type != sph::Type::Fluid) continue;
            neighbours.push_back(mirror[a].particle.id);
          }
        }
      }
    ...
    sycl::buffer<PartiallyAdvected<T, N, V>> advectedBuffer(mirror.size());
    sycl::buffer<size_t> hostGridTableBuffer(hostGridTable.data(), gridTableN);
    ...
    queue.submit([&](sycl::handler &H) {
      auto advected = advectedBuffer.template get_access<sycl::access::mode::write>(H);
      H.template copy(mirror.data(), advected);
    });
    wait();
    query();
    ...
    queue.submit([&](sycl::handler &H) {
      auto advected = advectedBuffer.template get_access<sycl::access::mode::read_write>(H);
      auto hostGridTable = hostGridTableBuffer.template get_access<sycl::access::mode::read>(H);
      H.parallel_for<kernel_diffuse<T, N>>(sycl::range<1>(xs.size()), [=](sycl::id<1> id) {
        if (advected[id].particle.type == sph::Type::Obstacle) return;
        int nNeighbours = 0;
        V<4, N> mixture{};
        foreach_grid(advected[id].zIndex, hostGridTable, gridTableN, [&](size_t b) {
        ...
        if (nNeighbours != 0) {
          auto a = advected[id].particle.colour;
          auto b = (mixture / N(nNeighbours)) * N(1.33);
          auto out = sycl::mix(a, b, config.dt / N(750.0));
          advected[id].particle.colour = sycl::clamp(out, N(0.03), N(1.0));
        }
      });
    });
    wait();
    ...
    for (size_t itr = 0; itr < config.iteration; ++itr) {
      ...
      queue.submit([&](sycl::handler &H) {
        ...
        H.parallel_for<kernel_lambda<T, N>>(sycl::range<1>(xs.size()), [=](sycl::id<1> id) {
          if (advected[id].particle.type == sph::Type::Obstacle) {
            advected[id].lambda = 0;
            return;
          }
          V<3, N> norm2V{};
          N rho = 0;
          foreach_grid(advected[id].zIndex, hostGridTable, gridTableN, [&](size_t b) {
            const N r = sycl::distance(advected[id].pStar, advected[b].pStar);
            norm2V +=
                spikyKernelGradient(advected[id].pStar, advected[b].pStar, r, h, SpikyKernelFactor) * N(RHO_RECIP);
            rho += advected[id].particle.mass * poly6Kernel(r, Poly6Factor, h);
          });
          N norm2 = sycl::dot(norm2V, norm2V);
          N Ci = (rho / RHO - N(1));
          advected[id].lambda = -Ci / (norm2 + CFM_EPSILON);
        });
      });
      wait();
      ...
      queue.submit([&](sycl::handler &H) {
        ...
        H.parallel_for<kernel_delta<T, N>>(sycl::range<1>(xs.size()), [=](sycl::id<1> id) {
          if (advected[id].particle.type == sph::Type::Obstacle) return;
          V<3, N> deltaPAcc{};
          foreach_grid(advected[id].zIndex, hostGridTable, gridTableN, [&](size_t b) {
            const N r = sycl::distance(advected[id].pStar, advected[b].pStar);
            const N corr = -CorrK * sycl::pow(poly6Kernel(r, Poly6Factor, h) / P6DeltaQ, N(CorrN));
            const N factor = (advected[id].lambda + advected[b].lambda + corr) / RHO;
            deltaPAcc += spikyKernelGradient(advected[id].pStar, advected[b].pStar, r, h, SpikyKernelFactor) * factor;
          });
          advected[id].deltaP = deltaPAcc;
          V<3, N> pos = (advected[id].pStar + advected[id].deltaP) * config.scale;
          pos = min(config.maxBound, max(config.minBound, pos)); // clamp to extent
          advected[id].pStar = pos / config.scale;
        });
      });
      wait();
    }
    ...
    queue.submit([&](sycl::handler &H) {
      auto advected = advectedBuffer.template get_access<sycl::access::mode::read_write>(H);
      H.parallel_for<kernel_finalise<T, N>>(sycl::range<1>(xs.size()), [=](sycl::id<1> id) {
        if (advected[id].particle.type == sph::Type::Obstacle) return;
        const auto deltaX = advected[id].pStar -
                            advected[id].particle.position / config.scale;
        advected[id].particle.position = advected[id].pStar * config.scale;
        advected[id].particle.velocity = (deltaX * (N(1) / config.dt) +
                                          advected[id].particle.velocity) *
                                         N(VD);
      });
    });
    ...
    if (config.surface) {
      ...
      sycl::buffer<V<4, N>> latticePNsBuffer(latticePNs.data(), latticeN);
      sycl::buffer<V<4, N>> latticeCsBuffer(latticeCs.data(), latticeN);

      queue.submit([&](sycl::handler &H) {
        auto advected = advectedBuffer.template get_access<sycl::access::mode::read>(H);

        auto latticePNs = latticePNsBuffer.template get_access<sycl::access::mode::read_write>(H);
        auto latticeCs = latticeCsBuffer.template get_access<sycl::access::mode::read_write>(H);
        auto hostGridTable = hostGridTableBuffer.template get_access<sycl::access::mode::read>(H);

        H.parallel_for<kernel_mc_lattice<T, N>>(
            sycl::range<3>(sampleSize.x(), sampleSize.y(), sampleSize.z()), [=](sycl::id<3> id) {
              ...
              foreach_grid(hostGridTable, gridTableN, offsets, [&](size_t b) {
                if (advected[b].particle.type != sph::Type::Obstacle &&
                  ...
                  const N denominator = sycl::pow(len, config.surface->particleInfluence);
                  v += (config.surface->particleSize / denominator);
                  normal += (-config.surface->particleInfluence) * config.surface->particleSize * (l / denominator);
      ...
      sycl::buffer<size_t> partialSumsBuffer(num_group);
      ...
      queue.submit([&](sycl::handler &H) {
        auto latticePNs = latticePNsBuffer.template get_access<sycl::access::mode::read>(H);
        auto partialSums = partialSumsBuffer.template get_access<sycl::access::mode::write>(H);
      ...
      auto r = partialSumsBuffer.template get_access<sycl::access::mode::read>();
      size_t numTrigs = 0;
      for (size_t i = 0; i < num_group; ++i)
        numTrigs += r[i];

      std::cout << "Vol=" << marchVolume << "  " << numTrigs << " "
                << "\n";
      ...
      if (numTrigs != 0) {
        ...
        sycl::buffer<V<3, N>> outVxsBuffer(outVxs.data(), outVxs.size());
        sycl::buffer<V<3, N>> outNxsBuffer(outNxs.data(), outNxs.size());
        sycl::buffer<V<4, N>> outCxsBuffer(outCxs.data(), outCxs.size());
        ...
        queue.submit([&](sycl::handler &H) {
          ...
          auto outVxs = outVxsBuffer.template get_access<sycl::access::mode::write>(H);
          auto outNxs = outNxsBuffer.template get_access<sycl::access::mode::write>(H);
          auto outCxs = outCxsBuffer.template get_access<sycl::access::mode::write>(H);
          auto latticeCs = latticeCsBuffer.template get_access<sycl::access::mode::read>(H);
          ...
          H.parallel_for<kernel_mc<T, N>>(sycl::range<1>(marchVolume), [=](sycl::id<1> id) {
            ...
            for (int i = 0; i < 8; ++i) {
              ...
              values[i] = point.x();
              offsets[i] =
                  (minExtent + ((offset).template convert<N, sycl::rounding_mode::automatic>() * step)) * config.scale;
            ...
            for (size_t i = 0; TriTable[ci][i] != 255; i += 3) {
              ...
              if (trigIndex >= numTrigs) {
                out << "Idx =" << trigIndex << cl::sycl::endl;
              } else {
        ...
        wait();
        queue.submit([&](sycl::handler &H) {
          auto outVxs_ = outVxsBuffer.template get_access<sycl::access::mode::read>(H);
          H.template copy(outVxs_, outVxs.data());
        });
        wait();

        queue.submit([&](sycl::handler &H) {
          auto outNxs_ = outNxsBuffer.template get_access<sycl::access::mode::read>(H);
          H.template copy(outNxs_, outNxs.data());
        });
        wait();

        queue.submit([&](sycl::handler &H) {
          auto outCxs_ = outCxsBuffer.template get_access<sycl::access::mode::read>(H);
          H.template copy(outCxs_, outCxs.data());
        });
        wait();
      }
    ...
    //
    sycl::buffer<sph::Particle<T, N, V>> sourceBuffer(xs.data(), xs.size());
    queue.submit([&](sycl::handler &H) {
      auto source = sourceBuffer.template get_access<sycl::access::mode::write>(H);
      auto advected = advectedBuffer.template get_access<sycl::access::mode::read>(H);
      H.parallel_for<kernel_write_back<T, N>>(sycl::range<1>(xs.size()),
                                              [=](sycl::id<1> id) { source[id] = advected[id].particle; });
    });
    wait();
    ...
    //        return fluid::Result<T, N, V>{geometry::ColouredMesh<N, V>{{}, {}, {}}, {}};
  }
};
